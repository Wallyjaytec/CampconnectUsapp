...
class OrderAddress {
  ...
  factory OrderAddress.fromJson(Map<String, dynamic>? json) {
    final j = json ?? {};
    return OrderAddress(name: j['name']?.toString() ?? '', address: j['address']?.toString() ?? '', phone: j['phone']?.toString() ?? '', postalCode: j['postal_code']?.toString() ?? '', country: j['country']?.toString() ?? '', state: j['state']?.toString() ?? '', city: j['city']?.toString() ?? '');
  }
}

class ReturnStatus {
  final String status; final String klass; final String label;
  ReturnStatus({required this.status, required this.klass, required this.label});
  factory ReturnStatus.fromJson(Map<String, dynamic>? json) { final j = json ?? {}; return ReturnStatus(status: (j['status'] ?? j['return_status'])?.toString() ?? '', klass: j['class']?.toString() ?? '', label: j['label']?.toString() ?? ''); }
}

class TrackingItem {
  final String message; final String date;
  TrackingItem({required this.message, required this.date});
  factory TrackingItem.fromJson(Map<String, dynamic> j) => TrackingItem(message: j['message']?.toString() ?? '', date: j['date']?.toString() ?? '');
}

class ShopInfo {
  final String shopName; final String shopLogo; final String shopSlug; final String shopId;
  ShopInfo({required this.shopName, required this.shopLogo, required this.shopSlug, required this.shopId});
  factory ShopInfo.fromJson(Map<String, dynamic>? json) { final j = json ?? {}; return ShopInfo(shopName: j['shop_name']?.toString() ?? '', shopLogo: j['shop_logo']?.toString() ?? '', shopSlug: j['shop_slug']?.toString() ?? '', shopId: j['shop_id']?.toString() ?? ''); }
}

class PickupPointInfo {
  final int id; final String name; final String phone; final String location; final String zoneName;
  PickupPointInfo({required this.id, required this.name, required this.phone, required this.location, required this.zoneName});
  factory PickupPointInfo.fromJson(Map<String, dynamic> j) => PickupPointInfo(id: (j['id'] is num) ? j['id'].toInt() : int.tryParse(j['id']?.toString() ?? '') ?? 0, name: j['name']?.toString() ?? '', phone: j['phone']?.toString() ?? '', location: j['location']?.toString() ?? '', zoneName: j['zone_name']?.toString() ?? '');
}
