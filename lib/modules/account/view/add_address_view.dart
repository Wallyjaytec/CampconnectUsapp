...
class AddAddressView extends StatelessWidget {
    ...
    if (v.showName) {
      widgets.addAll([
        Obx(() => CustomTextField(
          ...
          errorText: c.nameError.value.isNotEmpty ? c.nameError.value : null,
          onChanged: (_) { if (c.nameError.value.isNotEmpty) c.nameError.value = ''; },
        )),
    ...
    if (v.showPhone) {
      widgets.addAll([
        Obx(() => CustomTextField(
          ...
          errorText: c.phoneError.value.isNotEmpty ? c.phoneError.value : null,
          onChanged: (_) { if (c.phoneError.value.isNotEmpty) c.phoneError.value = ''; },
        )),
        const SizedBox(height: 10),
      ]);
    }
    ...
    if (v.showLocation) {
      widgets.addAll([
        Obx(() => CustomTextField(
          ...
          errorText: c.countryError.value.isNotEmpty ? c.countryError.value : null,
        )),
        const SizedBox(height: 10),
        Obx(() => CustomTextField(
          controller: c.stateC,
          ...
          onTap: () {
            if (c.selectedCountry.value == null) {
              Get.snackbar('Select Country'.tr, 'Please select country first'.tr, snackPosition: SnackPosition.TOP, backgroundColor: AppColors.primaryColor, colorText: AppColors.whiteColor);
              return;
            }
            _openStateSheet(context, c);
          },
          suffix: const Icon(Iconsax.arrow_down_1_copy, size: 16),
          errorText: c.stateError.value.isNotEmpty ? c.stateError.value : null,
        )),
        const SizedBox(height: 10),
        Obx(() => CustomTextField(
          controller: c.cityC,
          ...
          onTap: () {
            if (c.selectedState.value == null) {
              Get.snackbar('Select State'.tr, 'Please select state first'.tr, snackPosition: SnackPosition.TOP, backgroundColor: AppColors.primaryColor, colorText: AppColors.whiteColor);
              return;
            }
            _openCitySheet(context, c);
          },
          suffix: const Icon(Iconsax.arrow_down_1_copy, size: 16),
          errorText: c.cityError.value.isNotEmpty ? c.cityError.value : null,
        )),
        const SizedBox(height: 10),
      ]);
    }
    ...
    if (v.showPostalCode) {
      widgets.addAll([
        Obx(() => CustomTextField(
          ...
          errorText: c.postalError.value.isNotEmpty ? c.postalError.value : null,
          onChanged: (_) { if (c.postalError.value.isNotEmpty) c.postalError.value = ''; },
        )),
        const SizedBox(height: 10),
      ]);
    }
    ...
    if (v.showAddress) {
      widgets.addAll([
        Obx(() => CustomTextField(
          ...
          hint: 'Address'.tr,
          icon: Iconsax.location_copy,
          errorText: c.addressError.value.isNotEmpty ? c.addressError.value : null,
          onChanged: (_) { if (c.addressError.value.isNotEmpty) c.addressError.value = ''; },
        )),
        const SizedBox(height: 10),
      ]);
    }
  ...
  List<Widget> _buildShimmerForm(BuildContext context) {
    return [
      _shimmerField(context), const SizedBox(height: 10),
      _shimmerField(context), const SizedBox(height: 10),
      _shimmerField(context), const SizedBox(height: 10),
      _shimmerField(context), const SizedBox(height: 10),
      _shimmerField(context), const SizedBox(height: 10),
  ...
  Widget _shimmerField(BuildContext context) {
    ...
    return Shimmer.fromColors(
      ...
      highlightColor: isDark ? Theme.of(context).dividerColor : Colors.grey[100]!,
      child: Container(height: 50, decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12))),
    );
  }

  void _openCountrySheet(BuildContext context, AddressController c) {
    _openSelectSheet<CountryModel>(
      title: 'Select Country', itemsRx: c.countries, itemLabel: (x) => x.name,
      onSelected: (x) => c.onSelectCountry(x), isLoadingRx: c.isCountriesLoading,
  ...
  void _openStateSheet(BuildContext context, AddressController c) {
    _openSelectSheet<StateModel>(
      title: 'Select State'.tr, itemsRx: c.states, itemLabel: (x) => x.name,
      onSelected: (x) => c.onSelectState(x), isLoadingRx: c.isStatesLoading,
  ...
  void _openCitySheet(BuildContext context, AddressController c) {
    _openSelectSheet<CityModel>(
      title: 'Select City'.tr, itemsRx: c.cities, itemLabel: (x) => x.name,
      onSelected: (x) => c.onSelectCity(x), isLoadingRx: c.isCitiesLoading,
        ...
        child: Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12), boxShadow: [BoxShadow(color: Colors.black.withValues(alpha: 0.05), blurRadius: 6, offset: const Offset(0, 3))]),
                child: CustomTextField(controller: searchC, onChanged: (q) { filtered.assignAll(onSearch(q, itemsRx)); }, hint: 'Search'.tr, icon: Iconsax.search_normal_1_copy),
              ),
              const SizedBox(height: 8),
              Expanded(
                child: Obx(() {
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (ctx, i) {
                      ...
                      return ListTile(dense: true, title: Text(itemLabel(item)), onTap: () async {
