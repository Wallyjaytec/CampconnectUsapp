...
class _WebPayViewState extends State<WebPayView> {
      ...
      ..setNavigationDelegate(
        NavigationDelegate(
          ...
          onPageFinished: (url) async {
            if (!_completed) {
              await _controller.runJavaScript("document.body.style.display='none';");
            }
            _loading = true;
            setState(() {});
            if (_completed) return;
  ...
  bool _maybeHandleFromUrl(String url) {
    ...
    if (widget.successUrlContains != null && u.contains(widget.successUrlContains!.toLowerCase())) {
...
''';
    try {
      ...
      if (jsonStr.startsWith('"') && jsonStr.endsWith('"')) { jsonStr = jsonDecode(jsonStr) as String; }
      if (jsonStr.isEmpty) return;
      final decoded = jsonDecode(jsonStr);
      if (decoded is Map<String, dynamic>) {
        final success = decoded['success'] == true || decoded['success']?.toString().toLowerCase() == 'true';
        final status = (decoded['payment_status'] ?? '').toString().toUpperCase();
...
(function(){
  try{
    ...
    if (tooShort || hasAny(' ' + t + ' ', cues) || hasAny(' ' + b + ' ', cues)) { return 'ERROR_PAGE'; }
    return '';
  }catch(e){ return ''; }
})();
''';
  ...
  Future<void> _closeWithErrorUI({String title = 'Something went wrong', String subtitle = 'Unable to complete payment', String note = 'Please wait… Redirecting', required Duration delay, bool forceLoadHtml = false}) async {
    if (_completed || !mounted) return;
    _completed = true;
    _hardTimeoutTimer?.cancel();
    _graceTimer?.cancel();
  ...
  Future<void> _showInlineRedirectPage({String title = 'Thank you', String subtitle = 'Payment complete', String note = 'Please wait… Redirecting', String? brandLogoUrl, bool useLoadHtmlString = false}) async {
    final docHtml = '''<!doctype html><html><head><meta name="viewport" content="width=device-width, initial-scale=1"><title>$title</title><style>body{margin:0;background:#fff;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#111;}.wrap{min-height:100vh;display:flex;align-items:center;justify-content:center;padding:24px;box-sizing:border-box;}.card{width:100%;max-width:560px;border-radius:16px;box-shadow:0 10px 30px rgba(0,0,0,.08);padding:28px 24px;text-align:center;background:#fff;border:1px solid rgba(0,0,0,.06);}.logo{width:56px;height:56px;border-radius:12px;margin:0 auto 12px auto;object-fit:contain}h1{font-size:20px;margin:4px 0 6px;}p{margin:0;opacity:.75;}.note{margin-top:12px;font-size:14px;opacity:.85;}.spinner{width:22px;height:22px;margin:18px auto 0 auto;border-radius:50%;border:3px solid rgba(0,0,0,.12);border-top-color:rgba(0,0,0,.65);animation:spin .9s linear infinite;}@keyframes spin{to{transform:rotate(360deg)}}</style></head><body><div class="wrap"><div class="card">${brandLogoUrl != null && brandLogoUrl.trim().isNotEmpty ? '<img class="logo" src="$brandLogoUrl" />' : ''}<h1>$title</h1><p>$subtitle</p><p class="note">$note</p><div class="spinner"></div></div></div></body></html>''';
    if (useLoadHtmlString) {
      try { await _controller.loadHtmlString(docHtml); if (mounted) setState(() => _loading = false); } catch (_) {}
      return;
    }
    final js = '''(function(){try{document.head.innerHTML='';document.body.innerHTML='';document.body.style.margin='0';document.body.style.background='#ffffff';document.body.style.fontFamily='-apple-system, Segoe UI, Roboto, Helvetica, Arial, sans-serif';document.body.style.color='#111';document.body.style.display='block';const container=document.createElement('div');container.innerHTML=${jsonEncode(docHtml)};document.documentElement.innerHTML=container.innerHTML;}catch(e){}})();''';
    try { await _controller.runJavaScript(js); if (mounted) setState(() => _loading = false); } catch (_) {}
  }
  ...
  Widget build(BuildContext context) {
    return PopScope(
      ...
      child: Scaffold(
        appBar: AppBar(
          title: Text('Complete Payment'.tr),
          actions: [IconButton(icon: const Icon(Icons.close), onPressed: () { if (!_completed) Navigator.of(context).pop(false); })],
        ...
        body: Stack(children: [
          ...
          if (!_completed && (_loading || _inGrace)) Positioned.fill(child: Container(color: Colors.white)),
        ]),
      ),
    );
  }
