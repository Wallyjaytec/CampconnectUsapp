...
class EditAddressView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Edit Address'.tr, style: const TextStyle(fontWeight: FontWeight.normal, fontSize: 18)),
      ),
      body: RefreshIndicator(
        onRefresh: c.refreshFormConfig,
        child: Obx(() => ListView(padding: const EdgeInsets.all(12), physics: const AlwaysScrollableScrollPhysics(), children: [
      ...
      bottomNavigationBar: SafeArea(
        ...
        child: Obx(() {
          ...
          return SizedBox(height: 48, width: double.infinity, child: ElevatedButton(onPressed: disableBtn ? null : c.submitUpdate, child: submitLoading ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(strokeWidth: 2)) : Text('Update Address'.tr)));
        }),
      ),
    );
  }
  ...
  List<Widget> _buildRealForm(BuildContext context, EditAddressController c) {
    ...
    if (v.showName) { widgets.addAll([CustomTextField(controller: c.nameC, hint: 'Name'.tr, icon: Iconsax.user_copy), const SizedBox(height: 10)]); }
    if (v.showPhone) { widgets.addAll([CustomTextField(controller: c.phoneC, hint: 'Phone Number'.tr, icon: Iconsax.call_copy, keyboardType: TextInputType.phone), const SizedBox(height: 10)]); }
    if (v.showLocation) {
      widgets.addAll([
        CustomTextField(controller: c.countryC, hint: 'Country'.tr, icon: Iconsax.global_copy, readOnly: true, onTap: () => _openCountrySheet(context, c), suffix: const Icon(Iconsax.arrow_down_1_copy, size: 16)), const SizedBox(height: 10),
        CustomTextField(controller: c.stateC, hint: 'State'.tr, icon: Iconsax.location_copy, readOnly: true, onTap: () { if (c.selectedCountry.value == null) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Please select country first'.tr), backgroundColor: AppColors.primaryColor, behavior: SnackBarBehavior.floating, margin: const EdgeInsets.all(16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), duration: const Duration(seconds: 2))); return; } _openStateSheet(context, c); }, suffix: const Icon(Iconsax.arrow_down_1_copy, size: 16)), const SizedBox(height: 10),
        CustomTextField(controller: c.cityC, hint: 'City'.tr, icon: Iconsax.route_square_copy, readOnly: true, onTap: () { if (c.selectedState.value == null) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Please select state first'.tr), backgroundColor: AppColors.primaryColor, behavior: SnackBarBehavior.floating, margin: const EdgeInsets.all(16), shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)), duration: const Duration(seconds: 2))); return; } _openCitySheet(context, c); }, suffix: const Icon(Iconsax.arrow_down_1_copy, size: 16)), const SizedBox(height: 10),
      ]);
    }
    if (v.showPostalCode) { widgets.addAll([CustomTextField(controller: c.postalC, hint: 'Postal Code'.tr, icon: Iconsax.hashtag_1_copy, keyboardType: TextInputType.number), const SizedBox(height: 10)]); }
    if (v.showAddress) { widgets.addAll([CustomTextField(controller: c.addressC, hint: 'Address'.tr, icon: Iconsax.location_copy), const SizedBox(height: 10)]); }
    widgets.addAll([
      Obx(() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_sectionHeader('Status'.tr), RadioGroup<int>(groupValue: c.status.value, onChanged: (v) { if (v == null) return; c.status.value = v; }, child: _radioPair())])), const SizedBox(height: 8),
      Obx(() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_sectionHeader('Default Shipping Address'.tr), RadioGroup<int>(groupValue: c.defaultShipping.value, onChanged: (v) { if (v == null) return; c.defaultShipping.value = v; }, child: _radioPair())])), const SizedBox(height: 8),
      Obx(() => Column(crossAxisAlignment: CrossAxisAlignment.start, children: [_sectionHeader('Default Billing Address'.tr), RadioGroup<int>(groupValue: c.defaultBilling.value, onChanged: (v) { if (v == null) return; c.defaultBilling.value = v; }, child: _radioPair())])), const SizedBox(height: 10),
    ]);
    if (widgets.isEmpty) widgets.add(Text('No address fields enabled from server settings'.tr));
    return widgets;
  }

  List<Widget> _buildShimmerForm(BuildContext context) => [_shimmerField(context), const SizedBox(height: 10), _shimmerField(context), const SizedBox(height: 10), _shimmerField(context), const SizedBox(height: 10), _shimmerField(context), const SizedBox(height: 10), _shimmerField(context), const SizedBox(height: 10), _shimmerField(context), const SizedBox(height: 16), _shimmerRadioRow(context), const SizedBox(height: 8), _shimmerRadioRow(context), const SizedBox(height: 8), _shimmerRadioRow(context)];

  Widget _shimmerField(BuildContext context) {
    final isDark = Get.theme.brightness == Brightness.dark;
    return Shimmer.fromColors(baseColor: isDark ? Colors.grey[800]! : Colors.grey[300]!, highlightColor: isDark ? Colors.grey[600]! : Colors.grey[100]!, child: Container(height: 52, decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12))));
  }

  Widget _shimmerRadioRow(BuildContext context) {
    final isDark = Get.theme.brightness == Brightness.dark;
    return Shimmer.fromColors(baseColor: isDark ? Colors.grey[800]! : Colors.grey[300]!, highlightColor: isDark ? Colors.grey[600]! : Colors.grey[100]!, child: Row(children: [Expanded(child: Container(height: 32, decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(8)))), const SizedBox(width: 8), Expanded(child: Container(height: 32, decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(8))))]));
  }

  Widget _sectionHeader(String title) => Row(children: [Expanded(child: Text(title, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.normal)))]);
