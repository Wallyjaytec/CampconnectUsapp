...
class RefundRequestDetailsView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    Future<void> copyRefundId(String code) async {
      ...
      Get.snackbar('Copied'.tr, 'Refund ID copied'.tr, snackPosition: SnackPosition.TOP, margin: const EdgeInsets.all(12), duration: const Duration(seconds: 2), backgroundColor: AppColors.primaryColor, colorText: AppColors.whiteColor);
    }

    return Scaffold(
      appBar: AppBar(automaticallyImplyLeading: false, leadingWidth: 44, titleSpacing: 0, leading: Material(color: Colors.transparent, shape: const CircleBorder(), clipBehavior: Clip.antiAlias, child: IconButton(onPressed: () => Get.back(), icon: const Icon(Iconsax.arrow_left_2_copy, size: 20), splashRadius: 20)), centerTitle: false, title: Text('Refund Details'.tr, style: const TextStyle(fontSize: 18)), actionsPadding: const EdgeInsetsDirectional.only(end: 10), actions: const [SearchIconWidget(), NotificationIconWidget(), CartIconWidget()], elevation: 0),
      body: Obx(() {
        if (c.isLoading.value && c.details.value == null) return _fullScreenShimmer(context);
        final err = c.error.value;
        if (err != null) return Center(child: Padding(padding: const EdgeInsets.all(24), child: Column(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.error_outline, size: 48, color: Colors.red), const SizedBox(height: 12), Text(err, textAlign: TextAlign.center), const SizedBox(height: 12), ElevatedButton(onPressed: () => c.load(refundId), child: Text('Retry'.tr))])));
        final d = c.details.value!;
        final step = c.currentStepByLabel(d.returnStatus, d.paymentStatus);
        return RefreshIndicator(onRefresh: () => c.refreshNow(refundId), child: ListView(padding: EdgeInsets.zero, children: [
  ...
  Widget _headerCard(d, Function(String) copyRefundId) => Container(decoration: BoxDecoration(color: Get.theme.brightness == Brightness.dark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10), margin: const EdgeInsets.only(top: 10, left: 12, right: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [Expanded(child: Text('${'Refund ID'.tr} : ${d.refundCode}', style: const TextStyle(fontWeight: FontWeight.w600), overflow: TextOverflow.ellipsis)), IconButton(tooltip: 'Copy Refund ID'.tr, visualDensity: VisualDensity.compact, icon: const Icon(Iconsax.copy_copy, size: 18), onPressed: () => copyRefundId(d.refundCode))]),
    Text('${'Total'.tr} : ${formatCurrency(double.tryParse(d.totalAmount) ?? 0, applyConversion: true)}', style: const TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
    Text('${'Returned on'.tr} ${d.refundDate}', style: TextStyle(fontSize: 13, color: Colors.grey.shade700)),
  ]));

  Widget _statusCard(BuildContext context, d, int step, c) => Container(decoration: BoxDecoration(color: Get.theme.brightness == Brightness.dark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10), margin: const EdgeInsets.only(top: 10, left: 12, right: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Row(children: [Expanded(child: _kvSmall('Return'.tr, _titleCase(d.returnStatus))), const SizedBox(width: 8), Expanded(child: Align(alignment: Alignment.centerRight, child: _kvSmall('Payment'.tr, _titleCase(d.paymentStatus), color: _statusColor(d.paymentStatus))))]),
    const SizedBox(height: 12),
    _stepperWithShimmer(context, step),
    const SizedBox(height: 10),
    _trackingPanel(context: context, tracking: d.tracking, c: c),
    const SizedBox(height: 12),
    Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ClipRRect(borderRadius: BorderRadius.circular(8), child: CachedNetworkImage(imageUrl: _assetOrAbsolute(d.product.image), width: 56, height: 56, fit: BoxFit.cover, errorWidget: (_, __, ___) => Container(width: 56, height: 56, color: Colors.grey.shade300, alignment: Alignment.center, child: const Icon(Iconsax.gallery_remove_copy)))),
      const SizedBox(width: 10),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(d.product.name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600)), const SizedBox(height: 2), Row(children: [Expanded(child: Text('${'Qty'.tr}: ${d.product.quantity}', style: const TextStyle(fontSize: 12))), Text('${'Price'.tr}: ${formatCurrency(double.tryParse(d.product.price) ?? 0, applyConversion: true)}', style: const TextStyle(fontSize: 12))])])),
    ]),
  ]));

  Widget _infoCard(BuildContext context, d) => Container(decoration: BoxDecoration(color: Get.theme.brightness == Brightness.dark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12)), padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 10), margin: const EdgeInsets.only(top: 10, left: 12, right: 12, bottom: 12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
    Text('Refund Request Information'.tr, style: const TextStyle(fontWeight: FontWeight.w700)), const SizedBox(height: 8),
    _kvRow('Reason'.tr, d.refundReason), _kvRow('Note'.tr, d.note.isEmpty ? '-' : d.note), const SizedBox(height: 6),
    Text('Attachments'.tr, style: const TextStyle(fontWeight: FontWeight.w600)), const SizedBox(height: 8),
    if (d.attachments.isEmpty) Text('-', style: TextStyle(color: Colors.grey.shade700)) else _attachmentsGrid(context, d.attachments.map(_assetOrAbsolute).toList()),
  ]));

  Widget _fullScreenShimmer(BuildContext context) { final isDark = Theme.of(context).brightness == Brightness.dark; final base = isDark ? Colors.grey.shade800 : Colors.grey.shade300; final highlight = isDark ? Colors.grey.shade700 : Colors.grey.shade100; Widget block({double h = 120, EdgeInsets? m}) => Container(margin: m ?? const EdgeInsets.fromLTRB(12, 8, 12, 8), height: h, decoration: BoxDecoration(color: isDark ? AppColors.darkCardColor : AppColors.lightCardColor, borderRadius: BorderRadius.circular(12))); return Shimmer.fromColors(baseColor: base, highlightColor: highlight, child: SingleChildScrollView(child: Column(children: [block(h: 80, m: const EdgeInsets.fromLTRB(12, 12, 12, 8)), block(h: 220), block(h: 200)]))); }
  ...
  Widget _stepperWithShimmer(BuildContext context, int currentStep) {
    ...
    return Column(children: [
      ...
      FittedBox(fit: BoxFit.scaleDown, child: Row(spacing: 10, mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
  ...
  Widget _trackingPanel({required BuildContext context, required List<RefundTrackingItem> tracking, required RefundRequestDetailsController c}) {
    ...
    return Obx(() {
      ...
      return Container(margin: const EdgeInsets.only(top: 10), padding: const EdgeInsets.fromLTRB(12, 12, 12, 8), decoration: BoxDecoration(color: Theme.of(context).brightness == Brightness.dark ? Colors.white10 : Colors.black12, borderRadius: BorderRadius.circular(8)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 10),
        ...List.generate(itemsToShow.length, (i) { final t = itemsToShow[i]; final isLast = i == itemsToShow.length - 1; final isFirst = i == 0; return _RefundTimelineItem(date: t.date, message: t.message, isFirst: isFirst, isLast: isLast); }),
      ]));
    });
  }
...
class _RefundTimelineItem extends StatelessWidget {
  final String date; final String message; final bool isFirst; final bool isLast;
