...
class AddressController extends GetxController {
  ...
  void _clearErrors() {
    nameError.value = ''; phoneError.value = ''; countryError.value = '';
    stateError.value = ''; cityError.value = ''; postalError.value = ''; addressError.value = '';
  }

  Future<void> _initForm() async {
    try { await _loadFieldVisibility(); await ensureCountriesLoaded(); } 
    finally { isFormLoading.value = false; }
  }

  Future<void> _loadFieldVisibility() async {
    try { final map = await _settingsRepo.fetchSiteSettingsMap(); fieldVisibility.value = AddressFieldVisibility.fromSiteSettings(map); } 
    catch (_) { fieldVisibility.value = AddressFieldVisibility.defaults(); }
  }
  ...
  Future<void> refreshFormConfig() async {
    ...
    try {
      ...
      selectedCountry.value = null; selectedState.value = null; selectedCity.value = null;
      countryC.clear(); stateC.clear(); cityC.clear();
      _clearErrors();
  ...
  Future<void> onSelectCountry(CountryModel c) async {
    selectedCountry.value = c; countryC.text = c.name; countryError.value = '';
    phoneCode = _guessPhoneCode(c.code);
    selectedState.value = null; stateC.clear(); states.clear();
    selectedCity.value = null; cityC.clear(); cities.clear();
    await fetchStates(c.id);
  }

  Future<void> onSelectState(StateModel s) async {
    selectedState.value = s; stateC.text = s.name; stateError.value = '';
    selectedCity.value = null; cityC.clear(); cities.clear();
    await fetchCities(s.id);
  }

  Future<void> onSelectCity(CityModel c) async { selectedCity.value = c; cityC.text = c.name; cityError.value = ''; }

  Future<void> submitNewAddress() async {
    _clearErrors();
    final v = fieldVisibility.value;
    bool hasError = false;
    if (v.showName && nameC.text.trim().isEmpty) { nameError.value = 'Required'.tr; hasError = true; }
    if (v.showPhone && phoneC.text.trim().isEmpty) { phoneError.value = 'Required'.tr; hasError = true; }
    if (v.showLocation) {
      if (selectedCountry.value == null) { countryError.value = 'Required'.tr; hasError = true; }
      if (selectedState.value == null) { stateError.value = 'Required'.tr; hasError = true; }
      if (selectedCity.value == null) { cityError.value = 'Required'.tr; hasError = true; }
    }
    if (v.showPostalCode && postalC.text.trim().isEmpty) { postalError.value = 'Required'.tr; hasError = true; }
    if (v.showAddress && addressC.text.trim().isEmpty) { addressError.value = 'Required'.tr; hasError = true; }
    if (hasError) return;

    final co = selectedCountry.value; final st = selectedState.value; final ci = selectedCity.value;
    int countryId = co?.id ?? 0; int stateId = st?.id ?? 0; int cityId = ci?.id ?? 0;
  ...
  String? _guessPhoneCode(String code) {
    switch (code.toUpperCase()) { case 'BD': return '880'; case 'IN': return '91'; case 'US': return '1'; case 'GB': return '44'; default: return null; }
  }

  Future<void> initLoad() async { await fetchAddresses(); }
  ...
  @override
  void onClose() {
    nameC.dispose(); phoneC.dispose(); countryC.dispose(); stateC.dispose(); cityC.dispose(); postalC.dispose(); addressC.dispose();
    super.onClose();
  }
}
