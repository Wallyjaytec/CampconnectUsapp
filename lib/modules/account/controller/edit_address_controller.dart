...
class EditAddressController extends GetxController {
  ...
  Future<void> _initForm() async {
    try { await _loadFieldVisibility(); await _bootstrapSelections(); } 
  ...
  Future<void> _bootstrapSelections() async {
    try {
      ...
      if (initial.country != null) { final match = countries.firstWhereOrNull((e) => e.id == initial.country!.id); if (match != null) selectedCountry.value = match; }
      if (selectedCountry.value != null) {
        isStatesLoading.value = true;
        final slist = await addressRepo.getStates(countryId: selectedCountry.value!.id);
        states.assignAll(slist);
        isStatesLoading.value = false;
        if (initial.state != null) { final sm = states.firstWhereOrNull((e) => e.id == initial.state!.id); if (sm != null) selectedState.value = sm; }
      }
      if (selectedState.value != null) {
        isCitiesLoading.value = true;
        final ctlist = await addressRepo.getCities(stateId: selectedState.value!.id);
        cities.assignAll(ctlist);
        isCitiesLoading.value = false;
        if (initial.city != null) { final cm = cities.firstWhereOrNull((e) => e.id == initial.city!.id); if (cm != null) selectedCity.value = cm; }
      }
    } catch (_) {}
  }

  Future<void> onSelectCountry(CountryModel c) async {
    selectedCountry.value = c; countryC.text = c.name;
  ...
  Future<void> onSelectState(StateModel s) async {
    selectedState.value = s; stateC.text = s.name;
    selectedCity.value = null; cityC.clear(); cities.clear();
    isCitiesLoading.value = true;
  ...
  void onSelectCity(CityModel c) { selectedCity.value = c; cityC.text = c.name; }
  ...
  Future<void> submitUpdate() async {
    ...
    if (v.showLocation) { countryId = selectedCountry.value!.id; stateId = selectedState.value!.id; cityId = selectedCity.value!.id; } 
    else { countryId = initial.country?.id ?? 0; stateId = initial.state?.id ?? 0; cityId = initial.city?.id ?? 0; }
