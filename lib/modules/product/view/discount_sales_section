...
class _DiscountCardItem extends StatelessWidget {
  ...
  const _DiscountCardItem({required this.title, required this.imageUrl, required this.price, required this.oldPrice, required this.rating});
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Expanded(child: ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(10)), child: CachedNetworkImage(imageUrl: imageUrl, fit: BoxFit.cover, width: double.infinity, height: double.infinity))),
...
class _DiscountShimmer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Padding(padding: EdgeInsets.fromLTRB(10, 12, 10, 6), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ShimmerBox(height: 150, borderRadius: 10), SizedBox(height: 6),
      _HeaderShimmer(), SizedBox(height: 6), _ItemsRowShimmer(),
    ]));
  }
}
...
class _HeaderShimmer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const Row(children: [ShimmerBox(height: 16, width: 120, borderRadius: 6), Spacer(), ShimmerBox(height: 14, width: 140, borderRadius: 6)]);
  }
}
...
class _ItemsRowShimmer extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return const SizedBox(height: 180, child: Row(children: [
      Expanded(child: ShimmerBox(borderRadius: 10)), SizedBox(width: 8),
      Expanded(child: ShimmerBox(borderRadius: 10)), SizedBox(width: 8),
      Expanded(child: ShimmerBox(borderRadius: 10)), SizedBox(width: 8),
