...
class CheckoutController extends GetxController {
  ...
  // Per-product delivery mode tracking
  ...
  List<String> formattedAddressLines(CustomerAddress a) {
    final lastLine = [
      ...
      if (a.country?.name != null && a.country!.name.isNotEmpty) a.country!.name,
    ].join(', ');
    return [
      '${'Name'.tr}: ${a.name}',
  ...
  double get shippingFee {
    ...
    for (final it in _countedItems) {
      ...
      if (m == null) { if (op.defaultOption != null) sum += op.defaultOption!.cost; continue; }
      sum += m.cost;
    }
    return sum;
  }
  ...
  void _ingestIncomingItems() {
    ...
    if (arg is List<CartListItem>) { items.assignAll(arg); }
      ...
      for (final e in arg) {
        if (e is CartListItem) { list.add(e); }
        else if (e is CartApiItem) { list.add(_fromApiModel(e)); }
        else { try { list.add(CartListItem.fromJson(Map<String, dynamic>.from(e as Map))); } catch (_) {} }
      }
  ...
  Future<void> payWithWallet() async {
    ...
    try { await _submitOrder(useWallet: true); }
    finally { isWalletPaying.value = false; }
  }
  ...
  Future<void> _submitOrder({bool useWallet = false}) async {
    ...
    final hasHomeDelivery = productDeliveryMode.values.any((m) => m == DeliveryMode.home) || productDeliveryMode.isEmpty;
    if (hasHomeDelivery && selectedShipping == null) { _showSnackbar('Address'.tr, 'Please select a shipping address'.tr); return; }
    ...
    if (useWallet) { walletPayment = 1; paymentId = 2; }
      ...
      if (paymentId == null) { _showSnackbar('Payment'.tr, 'Please choose a payment method'.tr); return; }
      if (!_validateBankFields()) return;
    }

    final note = noteCtrl.text.trim();
    final body = <String, dynamic>{
      'payment_id': paymentId, 'note': note, 'wallet_payment': walletPayment, 'origin': 'app',
      'billing_address': (selectedBillingId.value ?? 0).toString(),
      'products': _productsJsonForCheckout(), 'coupon_discounts': jsonEncode(appliedCoupons),
    };
    ...
    if (!useWallet && isBankPaymentSelected) {
      ...
      if (path != null && path.isNotEmpty) body['receipt'] = path;
    }
    ...
    try {
      ...
      if (e is ApiHttpException) {
        try { final body = jsonDecode(e.body); if (body is Map && body['message'] != null) message = body['message'].toString(); } catch (_) {}
      }
  ...
  String _productsJsonForCheckout() {
    ...
    for (final it in _countedItems) {
      ...
      payload.add({'uid': uidVal, 'tax': tax, 'product_id': it.id, 'quantity': it.quantity, 'unitPrice': unitPrice, 'oldPrice': oldPrice, 'variant_code': it.variantCode, 'variant': it.variant, 'image': it.image, 'shipping_cost': method?.cost ?? 0.0, 'shipping_rate_id': method?.id ?? 0, 'attatchment': attachmentId});
    }
    return jsonEncode(payload);
  }

  Future<void> _refreshShippingOptions() async {
    int location = 0; String? postCode; String shippingType;
    final ship = selectedShipping;
    if (ship == null || items.isEmpty) { _clearOptionsState(); return; }
    ...
    try {
      ...
      final map = await _checkoutRepo.fetchShippingOptions(location: location, postCode: postCode, shippingType: shippingType, productsJsonString: _productsJsonString());
      if (Get.context == null) return;
      final parsed = ShippingOptionsResponse.fromJson(map);
      if (parsed.success != true) throw Exception('server returned success=false');
      ...
      for (final nap in parsed.notAvailableProducts) {
        ...
        if (uid != null && uid.isNotEmpty) { notAvailableUids.add(uid); }
        else { final id = (nap['id'] is num) ? (nap['id'] as num).toInt() : int.tryParse('${nap['id']}') ?? -1; final hit = items.firstWhereOrNull((i) => i.id == id); if (hit != null) notAvailableUids.add(hit.uid); }
      }
      for (final op in parsed.options) {
        String uid = op.productUid;
        if (uid.isEmpty || !items.any((i) => i.uid == uid)) { final fallbackUid = _uidByProductId[op.productId]; if (fallbackUid != null) uid = fallbackUid; }
        if (uid.isEmpty || !items.any((i) => i.uid == uid)) continue;
        optionsByUid[uid] = op; _taxByUid[uid] = op.tax;
        final def = op.defaultOption ?? (op.methods.isNotEmpty ? op.methods.first : null);
        if (def != null) selectedMethodByUid[uid] = def.id;
      }
    } catch (e) { optionsError.value = 'Failed to get shipping options'.tr; _clearOptionsState(); }
  ...
  Future<void> _refreshActivePaymentMethods() async {
    try {
      ...
      if (!resp.success) throw Exception('success=false');
      activePaymentMethods.assignAll(resp.data);
    } catch (e) { paymentError.value = 'Failed to load payment methods'.tr; }
    finally { isLoadingPayments.value = false; }
  }

  Future<void> _loadWalletSummary() async {
    try {
      isLoadingWallet.value = true; walletError.value = '';
      final summary = await _walletRepo.fetchWalletSummary();
      if (Get.context == null) return;
      walletAvailable.value = summary.totalAvailable.toDouble();
    } catch (e) { walletError.value = 'Failed to load wallet balance'.tr; walletAvailable.value = null; }
    finally { isLoadingWallet.value = false; }
  }
  ...
  Future<void> setShipping(int? id) async {
    ...
    if (billingMode.value == BillingMode.sameAsShipping) selectedBillingId.value = id;
    isScreenLoading.value = true;
    await _refreshShippingOptions(); await _refreshActivePaymentMethods(); await _loadWalletSummary();
    isScreenLoading.value = false;
  }
  ...
  Future<void> setBillingMode(BillingMode mode) async {
    ...
    if (mode == BillingMode.sameAsShipping) { selectedBillingId.value = selectedShippingId.value; }
    else { if (selectedBillingId.value == null && activeAddresses.isNotEmpty) selectedBillingId.value = activeAddresses.first.id; }
  }
  ...
  Future<void> addNewAddress(BuildContext context) async {
    ...
    if (result == true) {
      isScreenLoading.value = true; await _loadAddresses();
      if (!context.mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Address added successfully'.tr), backgroundColor: AppColors.primaryColor, behavior: SnackBarBehavior.floating));
    }
  }
  ...
  bool get isBankPaymentSelected {
    ...
    return m.name.toLowerCase().contains('bank') || (m.instruction ?? '').toLowerCase().contains('bank');
  }
  ...
  void selectShippingFor(String uid) {
    ...
    final context = Get.context; if (context == null) return;
    showDialog(
      context: context,
  ...
  void removeItemByUid(String uid) {
    ...
    notAvailableUids.remove(uid); selectedMethodByUid.remove(uid); optionsByUid.remove(uid); _taxByUid.remove(uid);
    productDeliveryMode.remove(uid); productPickupId.remove(uid);
    if (removed != null) _uidByProductId.remove(removed.id);
  }
  ...
  Future<void> _handleOrderResponse(Map<String, dynamic> resp) async {
    ...
    switch (result?.status) {
      case PaymentPageResultStatus.success: await _afterOrderSuccess(orderId);
      case PaymentPageResultStatus.cancelled: _showSnackbar('Payment'.tr, result?.message ?? 'Payment was cancelled'.tr);
      case PaymentPageResultStatus.failed: _showSnackbar('Payment'.tr, result?.message ?? 'Payment failed'.tr);
      case PaymentPageResultStatus.timeout: _showSnackbar('Payment'.tr, result?.message ?? 'Payment page timed out'.tr);
      case PaymentPageResultStatus.error: _showSnackbar('Payment'.tr, result?.message ?? 'Could not open payment page'.tr);
      case null: _showSnackbar('Payment'.tr, 'Payment was not completed'.tr);
    }
  }
  ...
  String _extractRedirectUrl(Map<String, dynamic> resp) {
    final url = ((resp['redirect_url'] ?? resp['response_url'] ?? '').toString().trim());
    if (url.isEmpty || url.toLowerCase() == 'none' || url.toLowerCase() == 'null') return '';
    return url;
  }
  ...
  int? _extractOrderId(Map<String, dynamic> resp) {
    ...
    if (direct is String) { final parsed = int.tryParse(direct); if (parsed != null) return parsed; }
    try { final data = resp['data']; if (data is Map && data['order_id'] != null) { final v = data['order_id']; if (v is int) return v; if (v is String) return int.tryParse(v); } } catch (_) {}
    return null;
  }

  void _clearAfterOrder() {
    noteCtrl.clear(); resetBankForm(); selectedPaymentMethodId.value = null; _clearOptionsState();
    productDeliveryMode.clear(); productPickupId.clear();
    if (Get.isRegistered<CartController>()) Get.find<CartController>().clearAfterOrder();
  }
  ...
  bool _validateBankFields() {
    ...
    if (missing.isNotEmpty) { _showSnackbar('Bank Payment'.tr, '${missing.join(', ')} ${missing.length == 1 ? 'is' : 'are'} required for bank payment.'); return false; }
    return true;
  }
...
int? _extractAttachmentFileId(dynamic attachment) {
  ...
  if (attachment is String) { final s = attachment.trim(); if (s.isEmpty || s == 'null') return null; try { final decoded = jsonDecode(s); if (decoded is Map) { final v = decoded['file_id']; if (v is int) return v; if (v is String) return int.tryParse(v); } if (decoded is int) return decoded; } catch (_) { return int.tryParse(s); } }
  return null;
}
